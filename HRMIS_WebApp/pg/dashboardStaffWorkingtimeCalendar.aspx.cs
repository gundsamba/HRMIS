    ...
    public partial class dashboardStaffWorkingtimeCalendar : System.Web.UI.Page
    {
        ...
        public string GetAppointCalendar()
        {
            ...
            try
            {
                ...
                string qry = @"SELECT a.ST_ID, NVL(b.TP,a.TP) as TP, a.DT, a.MINTM, a.MAXTM, a.ISWORK, c.BEGINTM, c.ENDTM
FROM(
...
LEFT JOIN (
    SELECT TO_DATE(DT, 'YYYY-MM-DD') as DT, BEGINTM, ENDTM FROM ST_CHULUUTIME WHERE ISRECEIVED=1 AND STAFFS_ID=" + stid + @" AND TO_DATE(DT, 'YYYY-MM-DD') BETWEEN TO_DATE('" + year + "-" + month + "-01', 'YYYY-MM-DD') AND LAST_DAY(TO_DATE('" + year + "-" + month + @"-01', 'YYYY-MM-DD'))
) c ON a.DT=c.DT
ORDER BY a.DT";
                    ...
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        else
                        {
                            if (dr["TP"].ToString() == "0")
                            {
                                ...
                                if (dr["BEGINTM"].ToString() != "")
                                {
                                    if (dr["BEGINTM"].ToString() == "08:30") late = new DateTime(Int32.Parse(date[0]), Int32.Parse(date[1]), Int32.Parse(date[2]), Int32.Parse(dr["ENDTM"].ToString().Split(':')[0]), Int32.Parse(dr["ENDTM"].ToString().Split(':')[1]), 00);
                                    else if (dr["ENDTM"].ToString() == "17:30") early = new DateTime(Int32.Parse(date[0]), Int32.Parse(date[1]), Int32.Parse(date[2]), Int32.Parse(dr["BEGINTM"].ToString().Split(':')[0]), Int32.Parse(dr["BEGINTM"].ToString().Split(':')[1]), 00);
                                }

                                if (dr["MINTM"].ToString() == "00:00:00" && dr["MAXTM"].ToString() == "00:00:00")
                                {
                                    strTable += "\"title\":\"Тасалсан\",\"className\":\"bg-color-redLight\",\"description\":\"Ирсэн " + come.ToString("HH:mm:ss").Replace("00:00:00", "--:--:--") + "<br/>Явсан: --:--:--<br/>Хоцорсон: --:--:--<br/>Эрт явсан: --:--:--\",\"tm\":\"" + late.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "-" + early.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "\",";
                                }
                                else if (dr["MINTM"].ToString() != dr["MAXTM"].ToString())
                                {
                                    ...
                                    if (Convert.ToDouble(come.Subtract(late).Seconds) > 0)
                                    {
                                        ...
                                        {
                                            strTable += "\"title\":\"Хоцорсон\",\"className\":\"bg-color-yellow\",\"description\":\"Ирсэн " + come.ToString("HH:mm:ss") + "<br/>Явсан: " + go.ToString("HH:mm:ss") + "<br/>Хоцорсон: " + lateTimeSpan.ToString() + "<br/>Эрт явсан: " + earlyTimeSpan.ToString() + "\",\"tm\":\"" + late.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "-" + early.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "\",";
                                        }
                                        else
                                        {
                                            strTable += "\"title\":\"Хоцорсон\",\"className\":\"bg-color-yellow\",\"description\":\"Ирсэн " + come.ToString("HH:mm:ss") + "<br/>Явсан: " + go.ToString("HH:mm:ss") + "<br/>Хоцорсон: " + lateTimeSpan.ToString() + "<br/>Эрт явсан: --:--:--\",\"tm\":\"" + late.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "-" + early.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "\",";
                                        }
                                    }
                                    else
                                    {
                                        if (Convert.ToDouble(go.Subtract(early).Seconds) < 0) strTable += "\"title\":\"Цагтаа ирсэн\",\"className\":\"bg-color-blueLight\",\"description\":\"Ирсэн " + come.ToString("HH:mm:ss") + "<br/>Явсан: " + go.ToString("HH:mm:ss") + "<br/>Хоцорсон: --:--:--<br/>Эрт явсан: " + earlyTimeSpan.ToString() + "\",\"tm\":\"" + late.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "-" + early.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "\",";
                                        else strTable += "\"title\":\"Цагтаа ирсэн\",\"className\":\"bg-color-blueLight\",\"description\":\"Ирсэн " + come.ToString("HH:mm:ss") + "<br/>Явсан: " + go.ToString("HH:mm:ss") + "<br/>Хоцорсон: --:--:--<br/>Эрт явсан: --:--:--\",\"tm\":\"" + late.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "-" + early.ToString("t", CultureInfo.CreateSpecificCulture("es-ES")) + "\",";
                                    }
                                }
                                else
                                {
